  ...
  initCanvas() {
    ...
    function animate() {
      ...
      dots.forEach(dot => {
        ...
        // Button hover interaction - repel from button shape
        if (hoveredButton) {
          const buttonRect = hoveredButton.getBoundingClientRect();
          const buffer = 80; // Extra space around button
          
          // Check if dot is within the expanded button area
          ...
          if (isInButtonArea) {
            // Calculate distance to nearest edge of the button
            ...
            // Calculate repel direction based on which edge is closest
            let repelX = 0;
            let repelY = 0;
        ...
        // Mouse interaction - only repel within radius
...
// Update position
    ...
    // Function to check for button hover
